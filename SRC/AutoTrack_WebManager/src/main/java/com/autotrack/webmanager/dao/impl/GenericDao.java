...
public class GenericDao implements IGenericDao{
	...
	public void iniciarColecoes( Object objetoPai, String[] filhos ) throws SecurityException, NoSuchMethodException, IllegalArgumentException, IllegalAccessException, InvocationTargetException {
		...
		for ( String filho : filhos ) {
			...
			Method metodo = 
				objetoPai.getClass().getMethod( 
					"get" + filho.substring( 0, 1 ).toUpperCase() + filho.substring( 1 )
				);
			
			this.sessionFactory.getCurrentSession().load(( metodo.invoke( objetoPai ) ),( metodo.invoke( objetoPai ).hashCode() ));
		}
	}
	 ...
	 * @param exemplo: objeto Example
	 * */
    @Transactional(readOnly = true, propagation = Propagation.REQUIRED)
    public List getPeloExemplo(Object example) {
		
		Criteria criteria = this.sessionFactory.getCurrentSession().createCriteria(example.getClass());
	 ...
	 * Metodo para listar objetos semelhantes ao Object example
	 * @param exemplo: objeto Example
	 * @param isEnableLike: True se � para ativar o "Like" na consulta, false para desativar
	 * @param isIgnoreCase: True se � para ignorar mai�sculas e min�sculas na consulta, false para case sensitive
	 * */
    @Transactional(readOnly = true, propagation = Propagation.REQUIRED)
	 ...
	 * @param isEnableLike: True se � para ativar o "Like" na consulta, false para desativar
	 * @param isIgnoreCase: True se � para ignorar mai�sculas e min�sculas na consulta, false para case sensitive
	 * @param isExcludeZeros: True se � para ignorar valores zero, n�o adicionando na consulta
	 * @return
	 */
	 ...
	 * @param id O identificador deve ser informado quando for uma altera��o, em caso de inclus�o deve-se informar null
	 * @param campo Campo do objeto
	 * @param dado Dado do objeto
	 * @return boolean
	 */
    @Transactional(readOnly = true, propagation = Propagation.REQUIRED)
    public List getDadosDuplicados(Class classe, Long id, String campo, Object dado){

		Criteria criterio = this.sessionFactory.getCurrentSession().createCriteria(classe);
	 ...
	 * @param c Classe do objeto
	 * @param id O identificador deve ser informado quando for uma altera��o, em caso de inclus�o deve-se informar null
	 * @param campoPai Campo Pai do objeto
	 * @param pai Pai do objeto e tem que ser informado quando for um cadastrado de pai e filho
	 * @param campo Campo do objeto
	 * @param dado Dado do objeto
	 * @return boolean
	 */
    @Transactional(readOnly = true, propagation = Propagation.REQUIRED)
    public List getDadosDuplicados(final Class classe, final Long id, final String campoPai, final Object pai, final String campo, final Object dado){
